      ...
      {stats.map((stat, i) => {
        ...
        return (
          ...
          >
            ...
            {!stat.isBook && (
              <div className="absolute top-4 right-4 w-12 h-12 rounded-full bg-primary/10 border border-primary/20 flex items-center justify-center shadow-sm group-hover:bg-primary group-hover:text-primary-foreground transition-all duration-500 z-10">
                <span className="font-serif font-bold text-2xl leading-none">{stat.value}</span>
              </div>
            )}
            ...
            {stat.isBook && stat.coverImage && (
              <div className="absolute top-4 right-12 w-16 h-24 rounded-xl overflow-hidden shadow-md rotate-6 group-hover:rotate-0 group-hover:scale-110 transition-all duration-500 z-10">
                <img src={stat.coverImage} alt={String(stat.value)} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
              </div>
            )}
            
            <div className="mt-auto h-16 flex flex-col justify-end">
              <p className={cn(
                "font-bold text-muted-foreground transition-colors duration-300 group-hover:text-foreground",
                stat.isBook ? "text-sm mb-1" : "text-lg"
              )}>
                {stat.label}
              </p>
              {stat.isBook && (
                <p className="font-sans font-bold text-foreground text-2xl line-clamp-2 leading-tight group-hover:scale-105 origin-left transition-transform duration-300">
                  {stat.value}
                </p>
              )}
            </div>
